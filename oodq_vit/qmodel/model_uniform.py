...
from .transformer_uniform import Transformer
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        # Patch embedding
        Conv2d = Quant_conv2d(bitW = bitW)
        self.patch_embedding = Conv2d(in_channels, dim, kernel_size=(fh, fw), stride=(fh, fw))
        ...
        # Transformer
        self.transformer = Transformer(bitW = bitW, abitW = abitW, num_layers=num_layers, dim=dim, num_heads=num_heads, 
                                       ff_dim=ff_dim, dropout=dropout_rate)
        
        # Representation layer
        if representation_size and load_repr_layer:
            Linear = Quant_Linear(bitW = bitW)
            self.pre_logits = Linear(dim, representation_size)
