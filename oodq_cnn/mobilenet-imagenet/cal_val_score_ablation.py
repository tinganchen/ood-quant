...
import utils.score_ablation as scr
...
def main():
    ...
    if args.ood_method in ['dice_', 'line_']:
        ARCH = f'mobilenetv2_{args.ood_method}'
    else: 
        ARCH = 'mobilenetv2'
...
def get_score(inputs, model, clip_l=None, clip=None, m=None, s=None, alpha=None, logits=None):
    if args.ood_method == "msp":
        ...
        scores = scr.get_energy_score(inputs, model)
    elif args.ood_method == "line":
        scores = scr.get_shap_energy_score(inputs, model)
    return scores
...
def score(args, loader_test, model_t):
    ...
    for i, (inputs, _) in enumerate(loader_test, 1):
        ...
        if args.ood_method == 'react':
            scores, clip, m, s = get_score(inputs, model_t, clip=None, logits=None)
        elif args.ood_method == 'vim':
            ...
            scores, m, s = get_score(inputs, model_t)
