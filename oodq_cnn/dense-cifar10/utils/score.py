...
def get_quant_score(inputs, model, clip=None, m=None, s=None, logits=None):
    if logits is None:
        with torch.no_grad():
            _ = model(inputs)
    ...
    feat_q, (m, s) = activation_quantize_fn(args.abitW, feat, clip, m, s)
    ...
    return scores, clip, m, s

def get_quant_mixed_score(inputs, model, clip=None, 
                          m=None, s=None, 
...
def activation_quantize_fn(a_bit, x, clip, m=None, s=None):
    x_ = x.detach().cpu()
...
def activation_quantize_fn00(a_bit, x, clip, m=None, s=None):
    x_ = x.detach().cpu()
