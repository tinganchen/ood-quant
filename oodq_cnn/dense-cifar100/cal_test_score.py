...
from data import cifar100_data_test
...
def main():
    ...
    loader = cifar100_data_test.Data(args)
    ...
    model_t = import_module(f'model.{ARCH}').__dict__[args.target_model](100, 100, 12,
                                                                         reduction=0.5, 
                                                                         bottleneck=True, 
                                                                         dropRate=0.0, 
                                                                         normalizer=None, 
...
def get_score(inputs, model, clip=None, m=None, s=None, alpha=None, logits=None,
    ...
    if args.ood_method == "msp":
        ...
        scores = scr.get_quant_mixed_score(inputs, model, clip, m, s, 
...
def score(args, loader_test_id, loader_test_ood, model_t):
    ...
    if args.mixed == 'False':
        # ID
        out_f1 = os.path.join(args.job_dir, f"out_scores_{args.dataset}_{args.ood_method}.txt")
        ...
        if not os.path.isfile(out_f1):
            for _, (inputs, _) in enumerate(loader_test_id, 1):
                ...
                if args.ood_method == 'react':
                    ...
                    s, _, _, _ = get_score(inputs, model_t, 
        ...
        labels_info = os.path.join(args.job_dir,  
                                   f"labels_{args.dataset}_{args.ood_dataset}.txt")
        
        
        ood_m, ood_s, ood_n, id_m, id_s = None, None, None, None, None
        
        if not os.path.isfile(out_f):
            for i, (inputs, labels) in enumerate(loader_test_ood, 1):
